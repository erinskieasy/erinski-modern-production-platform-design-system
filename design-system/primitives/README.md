...
<div class="ds-select-menu" data-ds-select-menu>
  ...
  <div class="ds-select-popover" id="example-menu" role="listbox" hidden>
...
- Leading icons always sit inside an `18px` dark-gray circle.
...
Use `scroll-shell.css` when a panel needs fixed top and bottom regions with an internally scrolling middle. This is the same structure used by the side menu and command modal.

## Custom Scrollbars

Use `scrollbar.css` and `scrollbar.behavior.js` on any region that scrolls.

- Add `data-ds-scrollbar` to the scroll container.
